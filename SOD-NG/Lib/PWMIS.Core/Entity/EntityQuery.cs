...
namespace PWMIS.DataMap.Entity
{
    ...
    public class EntityQuery<T> where T : EntityBase, new()
    {
        ...
        public string FillParameter(OQL oql, T entity, out IDataParameter[] paras)
        {
            ...
            if (Parameters == null || Parameters.Count < 1) //不存在Where
            {
                ...
                sql = sql.Substring(0, whereIndex + tableName.Length) +
                      (string.IsNullOrEmpty(condition) ? " " : (" WHERE " + condition.Substring(" AND ".Length)))
    ...
    public class EntityQuery : IEntityQuery
    {
        ...
        /// <param name="reader">数据阅读器对象</param>
        /// <returns>实体类集合</returns>
        public static List<T> QueryList<T>(IDataReader reader) where T : class
        {
            var list = new List<T>();
            using (reader)
            {
                if (reader.Read())
                {
                    ...
                    if (t is EntityBase)
                    {
                        ...
                        do
                        {
                            ...
                            var entityNew = (EntityBase) entity.Clone();
        ...
        internal static int DeleteInnerByDB(EntityBase entity, CommonDB DB)
        {
            ...
            var sql = "DELETE FROM [" + entity.TableName + "] WHERE ";
            var values = "";
            var condition = "";
            var index = 0;
