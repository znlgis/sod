    ...
    public abstract class EntityBase : INotifyPropertyChanged, ICloneable, IEntity
    {
        ...
        public int MapFrom(object pocoClass)
        {
            ...
            for (var i = 0; i < fcount; i++)
            {
                if (accessors[i] != null)
                {
                    PropertyValues[i] = accessors[i].GetValue(pocoClass);
                    count++;
                }
            }
            return count;
        ...
        {
            var key = string.Format("{0}_{1}", tableName, fieldName);
        ...
        private EntityMapType _entityMap = EntityMapType.Table;
        ...
        {
            if (PropertyGetting != null)
            {
                PropertyGetting(this, new PropertyGettingEventArgs(name));
            }
        }
        ...
        protected internal void setProperty(string propertyFieldName, object Value)
        {
            ...
            for (var i = 0; i < PropertyNames.Length; i++)
            {
                if (string.Compare(PropertyNames[i], propertyFieldName, true) == 0)
                {
                    ...
                    OnPropertyChanged(new PropertyChangedEventArgs(propertyFieldName));
                    changedlist[i] = true;
                    return;
                }
            }
            ...
            var dictChengs = new Dictionary<string, bool>();
            for (var i = 0; i < PropertyNames.Length; i++)
            {
        ...
        {
            var key = string.Format("{0}_{1}", TableName, propertyFieldName);
