...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        var nc = new NotifyingArrayList<string>("ID");
        var nc1 = nc; //nc1=["ID"]
        var nc2 = nc.Add("ID2"); //nc2=["ID","ID2"],nc=["ID"]

        var meta = new EntityMetaData { IdentityName = "ID", TableName = "table1" };
        var meta2 = meta with { TableName = "table2" };
        meta.TableName = "table1_1";

        var ssl_a = new SharedStringList<SimpleEntity>();
        var ssl_b = new SharedStringList<SimpleEntity>();
        var ssl_c = new SharedStringList<SimpleEntity>();
        ...
        var str1 = ssl_c.First(); //获取共享实例数据 "A"
        Console.WriteLine("Shared Data:{0}", str1);

        //OldSimpleEntity ose = new OldSimpleEntity();
        var se = new SimpleEntity { Name = "name1" };
        var se2 = new SimpleEntity { Name = "name2" };
