...
namespace PWMIS.DataMap.Entity
{
    ...
    public class OQL : IOQL, IDisposable
    {
        ...
        private readonly List<string> selectedFieldNames = new();
        ...
        ///     字段堆栈
        /// </summary>
        protected internal Stack<TableNameField> fieldStack = new();
        ...
        protected internal List<TableNameField> selectedFieldInfo = new();
        ...
        {
            if (dictAliases == null) return string.Format(" [{0}]", tnf.Field);
        ...
        protected internal void TakeTwoStackFields<T>(T leftParaValue, object rightParaValue,
            out TableNameField leftField, out TableNameField rightField)
        {
            leftField = null;
            rightField = null;
            ...
            if (count == 1)
            {
                ...
                else
                {
                     ...
                     */
                    //
                    var Value = CommonUtil.ChangeType<T>(tnf.Entity.PropertyList(tnf.Field));
                    if (Equals(leftParaValue, Value))
                        leftField = tnf;
                    else if (Equals(rightParaValue, Value)) //增加对右边参数值的判断 2015.5.11
                        rightField = tnf;
                    else
                        leftField = tnf;
                }
            ...
            else if (count >= 2)
            {
                 ...
                 *    //如果是相等比较，推荐用这种方式实现上面的比较： cmpResult =cmp.EqualValue(userName);
                 */
                var Value = CommonUtil.ChangeType<T>(tnf1.Entity.PropertyList(tnf1.Field));

                if (Equals(Value, leftParaValue) && !Equals(Value, rightParaValue))
                {
                    //修改 2015.7.5
        ...
        private string ToInsertFromString(string sql)
        {
            ...
            foreach (var key in insertFromOql.Parameters.Keys) Parameters.Add(key, insertFromOql.Parameters[key]);
            return sql;
        }
        ...
        private string ToSelectString(string sql)
        {
                    ...
                    foreach (var str in selectedFieldNames)
                    {
                        ...
                        if (!GroupbyFieldNames.Contains(item))
                            throw new FormatException(
                                "如果使用GROUP BY 子句，那么在SELECT 子句中查询的列必须也在GROUP BY 子句中出现！错误的列：" + item);
                    }
                }
            }
            ...
            if (dictAliases != null) //有关联查询
            {
                ...
                foreach (var tnf in selectedFieldInfo)
                    if (dictAliases.TryGetValue(tnf.Entity, out aliases))
                        sql_fields +=
                            ",\t\r\n" + string.Format(" {0}.[{1}] AS [{0}_{1}]", aliases,
                                tnf.Field); //关联查询，此处可能需要考虑字段AS别名 问题
                    else
                ...
                if (sql_fields == "" && sqlFunctionString.Length == 0)
                {
                    ...
                    {
                        sql_fields = "M.*";
                        foreach (var str in dictAliases.Values) sql_fields += string.Format(",{0}.*", str);
                    }
                }
            }
            else
            ...
            if (PageEnable)
                ...
                if (OrderByPK || sql.IndexOf(" order by ", StringComparison.OrdinalIgnoreCase) <= 0)
                {
                    if (currEntity.PrimaryKeys == null || currEntity.PrimaryKeys.Count == 0)
                        throw new Exception("OQL 分页错误，没有指明分页标识字段，也未给当前实体类设置主键。");
                    PageField = string.Format("[{0}]", currEntity.PrimaryKeys.First());
                }
        ...
        public OQL With(string sqlLockType)
        {
            ...
            {
                throw new ArgumentException(sqlLockType + " 不是SQLSERVER 要求的锁定类型！请参见SqlServerLock 枚举定义。内部错误信息：" +
                                            ex.Message);
            }
        }
        ...
        {
            if (haveChildOql || haveJoinOpt)
                return "";
            return currEntity.TableName;
        }
    ...
    public class OQL1 : OQL4, IOQL1
    {
            ...
            foreach (var temp in fields) //比较字段是否在实体类中
                if (string.Compare(temp, para.FieldName, true) == 0)
                {
                    ...
                    if (!paras.ContainsKey(paraName))
                    {
                        ...
                        if (para.CompareType == enumCompare.IN)
                        {
                            paras.Remove(paraName);
                            str += " AND [" + temp + "]" + cmpType + " (" + para.FieldValue + " ) ";
                        }
        ...
        {
            var currFieldName = CurrentOQL.TakeStackFields().Split(new[] { ',' }, StringSplitOptions.None);
            var formatstring = string.Format(operatorString, currFieldName);
    ...
    public class OQL3 : OQL4, IOQL3
    {
        ...
        public OQL4 Having(OQLCompareFunc cmpFun)
        {
            ...
            if (!Equals(cmpResult, null)) CurrentOQL.oqlString += "\r\n             HAVING " + cmpResult.CompareString;
            return new OQL4(CurrentOQL);
        }
    }
    ...
    public class OQL4 : IOQL4
    {
        ...
        public OQLOrderType OrderBy(object field)
        {
            ...
            END.PageOrderDesc = false;

            var sqlFieldName = END.TakeOneStackFields().SqlFieldName;
            END.OrderByPK = END.currEntity.PrimaryKeys.Contains(sqlFieldName.Trim().TrimStart('[').TrimEnd(']'));
            END.oqlString += temp + sqlFieldName;
        ...
        public OQL4 OrderBy(object field, string orderType)
        {
            ...
            END.PageOrderDesc = strTemp == "desc";

            var temp = END.haveOrderBy ? "," : "\r\n                 ORDER BY ";
            var sqlFieldName = END.TakeOneStackFields().SqlFieldName;
            END.OrderByPK = END.currEntity.PrimaryKeys.Contains(sqlFieldName.Trim().TrimStart('[').TrimEnd(']'));
            END.oqlString += temp + sqlFieldName + " " + orderType;
            END.haveOrderBy = true;
            return this;
        }
        ...
        public OQL4 OrderBy(string[] orderStr)
        {
            ...
            var temp = END.haveOrderBy ? "," : "\r\n                 ORDER BY ";
            END.haveOrderBy = true;
            var entitys = END.GetAllUsedEntity();
    ...
    public class OQLOrderType
    {
        ...
        public OQL END => CurrentOQL4.END;
    }
    ...
    public class JoinEntity
    {
        ...
        public OQL On(params object[] fields)
        {
            if (fields.Length > 0)
            {
                ...
                {
                    var tnf1 = _mainOql.fieldStack.Pop();
                    var tnf2 = _mainOql.fieldStack.Pop();
                    var tnf1String = _mainOql.GetOqlFieldName(tnf1);
                    var tnf2String = _mainOql.GetOqlFieldName(tnf2);
                    _mainOql.oqlString += string.Format(" AND {0} = {1} ", tnf1String, tnf2String);
                }
            }
        ...
        {
            var cmpTypeStr = GetCompareTypeString(compareType);
            var tnfRight = _mainOql.fieldStack.Pop();
            var tnfLeft = _mainOql.fieldStack.Pop();
            LeftString = _mainOql.GetOqlFieldName(tnfLeft);
            RightString = _mainOql.GetOqlFieldName(tnfRight);
    ...
    public class OQLCondition
    {
        ...
        private OQLCondition subCondition(string logicType, string currFieldName, string compareType, object Value)
        {
                ...
                compareType == "<" || compareType == "<>" || compareType.StartsWith("like"))
            {
                ...
                tnf.FieldValue = Value;
                ConditionString += logicType + "[" + currFieldName + "] " + compareType + " " +
                                   CurrentOQL.CreateParameter(tnf);
            }
