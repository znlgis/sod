...
namespace PWMIS.DataMap.Entity
{
    ...
    public class GOQL<T> where T : class
    {
        ...
        private readonly T currentEntity;
        ...
        /// </summary>
        public int AllCount => currentOQL.PageWithAllRecordCount;

        public OQL END => currentOQL;
        ...
        {
            currentOQL.Limit(pageSize, pageNumber);
            return this;
        }
        ...
        {
            currentOQL.PageWithAllRecordCount = allCount;
            currentOQL.Limit(pageSize, pageNumber);
            return this;
        }
        ...
        {
            currentOQL.Limit(pageSize, pageNumber, autoAllCount);
            return this;
        }

        public GOQL<T> Print(out string sqlInfo)
        {
            sqlInfo = string.Format("SQL:{0}\r\n{1}", currentOQL, currentOQL.PrintParameterInfo());
            return this;
        }
        ...
        {
            return EntityQuery.QueryList<T>(currentOQL, db);
        }
        ...
        {
            var list = EntityQuery.QueryList<T>(currentOQL, db);
            var dtoList = new List<T1>();

            foreach (var item in list)
            {
                var dto = new T1();
                var entity = item as EntityBase;
                if (entity != null) entity.MapToPOCO(dto); //实体类到dto的映射
        ...
        {
            return EntityQuery.QueryObject<T>(currentOQL, db);
        }
    ...
    {
        private readonly GOQL<T> currentGOQL;
        private readonly OQL1 currentOQL1;
    ...
    public class GOQL2<T> where T : class
    {
        ...
        public GOQL<T> END { get; }

        public GOQL<T> OrderBy(OQLOrderAction<T> orderAct)
        {
            var currentOQL4 = new OQL4(END.currentOQL).OrderBy(orderAct);
            return END;
        }
    }
}