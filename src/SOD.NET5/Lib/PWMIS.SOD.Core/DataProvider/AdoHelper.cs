...
namespace PWMIS.DataProvider.Data
{
    ...
    {
        private static Dictionary<Type, MyFunc<IDataReader, int, object>> dictReaderDelegate;
        ...
        public IList<TResult> GetList<TResult>(MyFunc<IDataReader, TResult> fun, string sqlFormat,
        ...
        public static List<T> QueryList<T>(IDataReader reader) where T : class, new()
        {
            ...
            using (reader)
            {
                if (reader.Read())
                {
                    ...
                    var readerDelegates = DataReaderDelegate();
                    var getDataMethods = new MyFunc<IDataReader, int, object>[fcount];

                    var accessors = new INamedMemberAccessor[fcount];
                    var accessorMethod = new DelegatedReflectionMemberAccessor();
                    ...
                    do
                    {
                        ...
                        for (var i = 0; i < fcount; i++)
                            ...
                            {
                                var read = getDataMethods[i];
                                var value = read(reader, i);
        ...
        private static Dictionary<Type, MyFunc<IDataReader, int, object>> DataReaderDelegate()
        {
            ...
            {
                var dictReader = new Dictionary<Type, MyFunc<IDataReader, int, object>>();
                ...
                dictReader.Add(typeof(short), (reader, i) => reader.GetInt16(i));
                dictReader.Add(typeof(long), (reader, i) => reader.GetInt64(i));
        ...
        {
            if (parameters == null) return ExecuteDataReader(sqlFormat);
