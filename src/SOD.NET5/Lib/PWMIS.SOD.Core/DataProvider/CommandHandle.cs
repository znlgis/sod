...
namespace PWMIS.DataProvider.Data
{
    ...
    public class CommandExecuteLogHandle : ICommandHandle
    {
        ...
        public CommandExecuteLogHandle()
        {
            ...
            if (CommandLog.DataLogFile == null)
            {
                ...
                CommandLog.DataLogFile =
                    Path.Combine(logFolder, "sqllog_" + DateTime.Now.ToString("yyyyMMdd") + ".txt");
            }
        }

        public CommandLog CurrCommandLog { get; }


        public bool OnExecuting(CommonDB db, ref string SQL, CommandType commandType, IDataParameter[] parameters)
        {
            CurrCommandLog.ReSet();
            return true;
        }
        ...
        public DBMSType ApplayDBMSType => DBMSType.UNKNOWN;
    ...
    {
        private readonly MyCommandLogEntity logEntity;
            ...
            if (enable)
            {
                ...
                if (recordAffected > 0)
                    ...
                    if (BeforLog == null || (BeforLog != null && BeforLog(logEntity)))
                    {
                        ...
                        EntityQuery<MyCommandLogEntity>.Instance.Insert(logEntity, currDb);
                        enable = true;
                    }
        ...
        ///     获取当前处理器要应用的命令执行类型，只有符合该类型才会应用当前命令处理器
        /// </summary>
        public CommandExecuteType ApplayExecuteType => CommandExecuteType.ExecuteNonQuery;
    }
}