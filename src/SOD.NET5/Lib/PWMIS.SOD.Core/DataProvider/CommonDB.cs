...
namespace PWMIS.DataProvider.Data
{
    ...
    public abstract class CommonDB : IDisposable
    {
        ...
        public CommonDB()
        {
            ...
            EnableCommandHandle = true;
        }

        /// <summary>
        ///     事务计数器，如果大于0，表示当前查询在事务过程中
        /// </summary>
        public int TransactionCount => transCount;
        ...
        public string DataWriteConnectionString
        {
            get =>
                ...
                _writeConnString ?? ConnectionString;
        ...
        /// </summary>
        public bool OnErrorRollback { get; set; } = true;
        ...
        /// </summary>
        public bool OnErrorThrow { get; set; } = true;
        ...
        public ICommonLog Logger
        {
            get
            {
                if (_logger == null)
                    //第一个命令处理器就是日志处理器
                    _logger = ((CommandExecuteLogHandle)commandHandles[0]).CurrCommandLog.LogWriter;
                //上面必定有，注释下面代码
                /*
            ...
            set
            {
                ...
                foreach (var handle in commandHandles)
        ...
        /// </summary>
        public bool SqlServerCompatible { get; set; } = true;
        ...
        {
            if (db != null) return db.CurrentDBMSType;
        ...
        public static AdoHelper CreateInstance(string providerAssembly, string providerType)
        {
            ...
            if (cacheHelper.ContainsKey(key)) return (AdoHelper)Activator.CreateInstance(cacheHelper[key]);
        ...
        protected virtual IDbConnection GetConnection() //
        {
            ...
            //如果开启连接会话，则使用该连接
            if (sessionConnection != null) return sessionConnection;
            return null;
        }
        ...
        {
            Interlocked.Increment(ref transCount);
            ErrorMessage = "";
        ...
        public void Commit()
        {
            Interlocked.Decrement(ref transCount);
        ...
        public virtual int ExecuteNonQuery(string SQL, CommandType commandType, IDataParameter[] parameters)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (OnErrorThrow)
                    throw new QueryException(ErrorMessage, cmd.CommandText, commandType, parameters, inTransaction,
                        conn.ConnectionString, ex);
            }
            finally
            {
        ...
        public virtual int ExecuteInsertQuery(string SQL, CommandType commandType, IDataParameter[] parameters,
            ref object ID, string insertKey)
        {
            ...
            try
            {
                ...
                if (insertKey == "") insertKey = InsertKey;
        ...
        public virtual DataSet ExecuteDataSetWithSchema(string SQL, CommandType commandType,
        ...
        private readonly List<ICommandHandle> commandHandles;
        ...
        protected bool OnCommandExecuting(ref string sql, CommandType commandType, IDataParameter[] parameters,
            CommandExecuteType executeType = CommandExecuteType.ExecuteQuery)
        {
            if (EnableCommandHandle)
            {
                var isBreak = false;
                foreach (var handle in commandHandles)
                    if ((handle.ApplayExecuteType == CommandExecuteType.Any || handle.ApplayExecuteType == executeType)
                        && (handle.ApplayDBMSType == DBMSType.UNKNOWN || handle.ApplayDBMSType == CurrentDBMSType))
                    {
                        var flag = handle.OnExecuting(this, ref sql, commandType, parameters);
                        if (!flag)
                            isBreak = true;
                        ;
                    }
            ...
            if (EnableCommandHandle)
                foreach (var handle in commandHandles)
                    if ((handle.ApplayExecuteType == CommandExecuteType.Any || handle.ApplayExecuteType == executeType)
                        && (handle.ApplayDBMSType == DBMSType.UNKNOWN || handle.ApplayDBMSType == CurrentDBMSType))
                    {
                        var result = handle.OnExecuted(cmd, recordAffected);
                        if (handle is CommandExecuteLogHandle)
                            _elapsedMilliseconds = result;
                    }
        }
