...
namespace PWMIS.Core.Extensions
{
    ...
    {
        private static readonly Dictionary<Type, Dictionary<Type, ModelCast>> mCasters = new(256);
        private readonly List<CastProperty> mProperties = new();
        ...
        public ModelCast(Type sourceType, Type targetType)
        {
            ...
            foreach (var tp in targetProperties)
                ...
                //感谢网友 【落阳】提供修改建议
                if (sp.Name == tp.Name && (sp.PropertyType == tp.PropertyType ||
                                           Nullable.GetUnderlyingType(sp.PropertyType) == tp.PropertyType))
                {
        ...
        public class PropertyAccessorHandler
        {
            ...
            ///     Get访问器
            /// </summary>
            public Func<object, object[], object> Getter { get; }

            /// <summary>
            ///     Set访问器
            /// </summary>
            public Action<object, object, object[]> Setter { get; }
        }
    }
