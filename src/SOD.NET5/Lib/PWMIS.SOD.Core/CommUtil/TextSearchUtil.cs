...
namespace PWMIS.Core
{
    ...
    public class TextSearchUtil
    {
        ...
        public static Point SearchWordsIndex(string source, string words)
        {
            ...
            var firstMatchIndex = source.IndexOf(matchWrodArray[0] + ' ', 0, StringComparison.OrdinalIgnoreCase);
        ...
        public static Point SearchWordsLastIndex(string source, string words)
        {
            ...
            var matchWord = matchWrodArray.Length > 1 ? matchWrodArray[0] + " " : matchWrodArray[0];
            var firstMatchIndex = source.LastIndexOf(matchWord, source.Length - 1, StringComparison.OrdinalIgnoreCase);
            if (matchWrodArray.Length == 1 && firstMatchIndex != -1)
        ...
        /// <param name="targetIndex">所找到目标单词在源字符串中的位置索引，如果没有找到，返回-1</param>
        /// <returns>找到的新单词</returns>
        public static string FindNearWords(string inputString, int startIndex, int maxLength, char leftSqlSeparator,
            ...
            foreach (var c in inputString)
                ...
                else
                {
                    if (char.IsSeparator(c) || char.IsPunctuation(c))
                    {
                        ...
                        if (c == '*' || c == '.' || c == '_' || c == '-' || c == leftSqlSeparator ||
                            c == rightSqlSeparator)
                        {
