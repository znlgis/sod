...
namespace PWMIS.DataMap.Entity
{
    ...
    public class OQL : IOQL, IDisposable
    {
        ...
        /// </example>
        /// </summary>
        /// <typeparam name="T">实体类类型或者接口类型</typeparam>
        /// <param name="mapTableName">接口类型映射的表名称</param>
        /// <returns>OQL表达式</returns>
        public static GOQL<T> FromObject<T>(string mapTableName=null) where T : class
        {
        ...
        public OQL With(string sqlLockType)
        {
            ...
            {
                throw new ArgumentException(sqlLockType + " 不是SQLSERVER 要求的锁定类型！请参见SqlServerLock 枚举定义。内部错误信息："+ ex.Message );
            }
        }
