    ...
    public class EntityFields
    {
        ...
        */
        ...
        public string CreateTableColumnScript(AdoHelper db, EntityBase entity, string field)
        {
            ...
            {
                if (db.CurrentDBMSType == PWMIS.Common.DBMSType.SqlServer 
                    || db.CurrentDBMSType == PWMIS.Common.DBMSType.SqlServerCe
                    || db.CurrentDBMSType == PWMIS.Common.DBMSType.Dameng)
                {
                ...
                else if (db.CurrentDBMSType == PWMIS.Common.DBMSType.MySql )
                {
                    temp = temp + " AUTO_INCREMENT";
                      ...
                      || db.CurrentDBMSType == DBMSType.Kingbase)
                {
                    string seqName = entity.GetTableName() + "_" + entity.IdentityName + "_" + "seq";
                    temp = temp + " DEFAULT nextval('" + seqName.ToLower() + "'::regclass) NOT NULL";
                }
                else
                {
