 ...
 * 
 * 修改者：      时间：2024-5-16   
 * 修改说明：增加 Dameng 达梦数据库 自动创建表支持自增功能。
 * ========================================================================
*/
...
namespace PWMIS.DataMap.Entity
{
    ...
    public class EntityCommand
    {
        ...
        public string GetInsertKey()
        {
            ...
            else if ((this.currDb.CurrentDBMSType == PWMIS.Common.DBMSType.PostgreSQL
                   || this.currDb.CurrentDBMSType == PWMIS.Common.DBMSType.Kingbase)
                   && !string.IsNullOrEmpty(currEntity.IdentityName))
            {
                //2016.11.20增加此处代码, 2024.5.14增加Kingbase支持
...
";
                    ...
                    if ((this.currDb.CurrentDBMSType == PWMIS.Common.DBMSType.PostgreSQL
                        || this.currDb.CurrentDBMSType == PWMIS.Common.DBMSType.Kingbase)
                        && !string.IsNullOrEmpty(currEntity.IdentityName))
                    {
                        string seqName = currEntity.GetTableName() + "_" + currEntity.IdentityName + "_" + "seq";
                        string seq =
                            "CREATE SEQUENCE " + seqName.ToLower() + " INCREMENT 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1;";

                        script = seq + script;
                    }
                    /* 达梦数据库 以下创建序列的语法有问题，直接使用自增表的方式，参考官方文档：
                     * https://eco.dameng.com/document/dm/zh-cn/faq/faq-sql-gramm.html
                    else if (this.currDb.CurrentDBMSType == PWMIS.Common.DBMSType.Dameng && !string.IsNullOrEmpty(currEntity.IdentityName))
                    {
                        string seqName = currEntity.GetTableName() + "_" + currEntity.IdentityName + "_" + "seq";
                        string seq =
                            "CREATE SEQUENCE " + seqName.ToLower() + " INCREMENT BY 1 MINVALUE 1 MAXVALUE 20000000 START WITH 1 CACHE 1;";
