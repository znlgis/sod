...
namespace PWMIS.DataProvider.Data
{
    ...
    public abstract class CommonDB : IDisposable
    {
       ...
       /// </summary>
        ...
        /// <summary>
        /// 执行原始语句的查询，中间不会做任何执行前的语句内容替换，比如替换@字符和中括号。
        /// </summary>
        /// <param name="SQL">执行的SQL语句</param>
        /// <returns>返回受影响的行</returns>
        public int ExecuteOriginalStatement(string SQL)
        {
            ErrorMessage = "";
            IDbConnection conn = GetConnection();
            if (conn.State != ConnectionState.Open) //连接已经打开，不能切换连接字符串，感谢网友 “长的没礼貌”发现此Bug 
            ...
            catch (Exception ex)
            {
                ...
                if (OnErrorThrow)
                {
                    throw new QueryException(ErrorMessage, cmd.CommandText,  CommandType.Text, null, inTransaction, conn.ConnectionString, ex);
                }
            }
            finally
            {
        ...
        /// <summary>
        /// 执行插入数据的查询，如果有数据插入的标识列（例如自增列）则获取当前标识列的值
        /// </summary>
        /// <param name="SQL">插入数据的SQL</param>
        /// <param name="commandType">命令类型</param>
        /// <param name="parameters">参数数组</param>
        /// <param name="ID">要传出的本次操作的新插入数据行的标识列的值，如果没有获取到，则为-1</param>
