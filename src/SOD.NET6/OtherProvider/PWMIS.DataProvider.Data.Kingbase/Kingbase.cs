...
namespace PWMIS.DataProvider.Data
{
    ...
    /// Kingbase数据访问类
    /// </summary>
    public class Kingbase : AdoHelper
    {
        /// <summary>
        /// 默认构造函数
        /// </summary>
        ...
        {
            get { return PWMIS.Common.DBMSType.Kingbase; } 
        }
        ...
        protected override IDbConnection GetConnection()
        {
            ...
            if (conn == null)
            {
                conn = new KdbndpConnection(base.ConnectionString);
                //conn.Open ();
            }
            return conn;
        }
        ...
        {
            IDbDataAdapter ada = new KdbndpDataAdapter((KdbndpCommand)command);
            return ada;
        }
        ...
        {
            KdbndpParameter para = new KdbndpParameter();
            para.ParameterName = paraName;
            if(Value != null)
            {
                Type type = Value.GetType();
                //Kingbase内置有枚举类型，所以这里要处理.net应用程序的枚举类型
                if (type.IsEnum)
                {
                    para.KdbndpDbType = KdbndpTypes.KdbndpDbType.Integer;
                    para.Value = Convert.ToInt32(Value);
                }
                else
                {
                    para.Value = Value;
        ...
        {
            KdbndpConnection conn = (KdbndpConnection)this.GetConnection();
        ...
        {
            //由于函数定义语句较长，放到了资源文件中
            string sql = PWMIS.KingbaseClient.Resource.sql_function_args;
 ...
 WHERE specific_schema NOT IN
   ...
   AND type_udt_name != 'trigger';";
        ...
        public override System.Data.Common.DbConnectionStringBuilder ConnectionStringBuilder
        {
            get 
            {
                return new Kdbndp.KdbndpConnectionStringBuilder(this.ConnectionString);
            }
        }
        ...
        {
            get {
                return ((KdbndpConnectionStringBuilder)this.ConnectionStringBuilder).Username;
            }
        }
    }
}
