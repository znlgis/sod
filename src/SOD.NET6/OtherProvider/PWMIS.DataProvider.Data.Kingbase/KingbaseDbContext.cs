...
namespace PWMIS.KingbaseClient
{
    /// <summary>
    /// Kingbase数据库上下文，可以实现自动检查数据库，创建表，获取EntityQuery 泛型实例对象等功能，封装了AdoHelper的使用。
    /// </summary>
    public sealed class KingbaseDbContext : IDbContextProvider
    {
        ...
        public KingbaseDbContext(AdoHelper db)
        {
        ...
        {
            //创建表
            if (CurrentDataBase.CurrentDBMSType == PWMIS.Common.DBMSType.Kingbase)
            {
