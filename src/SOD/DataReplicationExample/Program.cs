...
namespace DataReplicationExample
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            var sourceDb = AdoHelper.CreateHelper("SourceDB");
            var targetDb = AdoHelper.CreateHelper("ReplicatedDB");
            ...
            //初始化数据库
            var sourceCtx = new TransLogDbContext(sourceDb);
            ...
            {
                var logHandle = new TransactionLogHandle();
                logHandle.BeforLog = log =>
                {
                    //只有在指定范围内的表并且符合对此表指定的操作行为，才会记录事务日志
                    log.LogTopic = "Test";
                    //定义操作策略
                    var tableStrategy = new Dictionary<string, SQLOperatType>
                    {
                        { "Table_User", SQLOperatType.Insert | SQLOperatType.Update }
                    };
                    //执行策略
                    foreach (var key in tableStrategy.Keys)
                        if (string.Compare(log.CommandName, key) == 0)
                        {
                            var sqlType = tableStrategy[key];
            ...
            //初始化目标数据库
            var targetCtx = new TransLogDbContext(targetDb);
            ...
            var count = sourceCtx.ReadLog(5, list =>
            {
                //复制数据到目标库
                foreach (var log in list)
                {
                    var result = targetCtx.DataReplication(log);
                    if (!result)
                    {
                        Console.WriteLine("****复制数据遇到错误，日志ID:{0}，命令：{1}，错误原因：{2}", log.CommandID, log.CommandText,
                            targetCtx.ErrorMessage);
                        return false;
                    }

                    Console.WriteLine("操作成功，日志ID:{0}，执行状态：{1}， 命令：{2}", log.CommandID, targetCtx.CurrentStatus,
                        log.CommandText);
                }
        ...
        {
            Console.WriteLine("----当前日志读取进度 ({0}/{1})={2}% -------------", e.ReadCount, e.AllCount,
                100 * e.ReadCount / e.AllCount);
        }
    }
}