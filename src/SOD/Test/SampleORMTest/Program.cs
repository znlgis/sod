...
继续请回车，退出请输入字母 Q ."
            ...
            //测试查询&&更新
            var zhang_san = new User { Name = "zhang san" };
            var dbZhangSan = EntityQuery<User>.QueryObject(
                OQL.From(zhang_san)
            ...
            if (dbZhangSan != null)
            {
                ...
                var updateCount = EntityQuery<User>.Instance.Update(dbZhangSan);
            ...
            var li_si = new User { Name = "li si", Pwd = "123" };
            var insertQ = OQL.From(li_si)
                .Insert(li_si.Name); //仅仅插入用户名，不插入密码

            //采用OQL方式插入指定的数据
            //同时演示事务使用方法
            var db = MyDB.GetDBHelperByConnectionName("local");
            var userQuery = new EntityQuery<User>(db);
            //db.BeginTransaction();
            //try
            //{
            ...
            {
                var zhang_yeye = new User { ID = 1000 + i, Name = "zhang yeye" + i, Pwd = "pwd" + i };
            ...
            //修改 测试数据----------------------------------------------------
            li_si.Pwd = "123123";
            count = context.Update(li_si); //采用 DbContext 方式更新数据

            li_si.Pwd = "123456";
            var updateQ = OQL.From(li_si)
