...
namespace SampleORMTest
{
    internal class UserLoginService
    {
        ...
        public bool Login(User user)
        {
            ...
            var dbUser = q.ToEntity<User>(); //ToEntity，OQL扩展方法 
        ...
        public bool Login1(User user)
        {
            ...
            var dbUser = EntityQuery<User>.QueryObject(q);
            return dbUser != null; //查询到用户实体类，表示登录成功
        }
        ...
        public bool Login6(string name, string pwd)
        {
            ...
            user.Name = name;
            user.Pwd = pwd;
            var result = EntityQuery<User>.Fill(user); //静态方法，使用默认的连接对象
            return result;
        }
        ...
        public List<User> FuzzyQueryUser(string likeName)
        {
            ...
            var users = EntityQuery<User>.QueryList(q);
            return users;
        }
    }
}