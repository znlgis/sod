...
namespace PWMIS.Core
{
    ...
    {
        private static readonly UniqueSequenceGUID UniqueId = new UniqueSequenceGUID();

        /// <summary>
        ///     缓存的SQLMAP实体类中映射的SQL，键为实体类的SQL查询名称
        /// </summary>
        public static Dictionary<string, string> CacheEntityMapSql { get; set; }
        ...
        {
            var so = assembly.GetManifestResourceStream(resourceName);
            if (so == null)
                throw new Exception("未找到指定的嵌入式资源: " + resourceName);
            var sr = new StreamReader(so);
        ...
        public static void ReplaceWebRootPath(ref string sourcePath)
        {
            ...
            if (!string.IsNullOrEmpty(sourcePath) && sourcePath.IndexOf('~') >= 0)
            {
                ...
                if (path != null && path.Length > 4 && path.EndsWith("bin", StringComparison.OrdinalIgnoreCase))
                    ...
                    appRootPath = path + Path.DirectorySeparatorChar;
        ...
        public static T ChangeType<T>(object Value)
        {
            ...
            if (currType.IsEnum) return (T)Convert.ChangeType(Value, TypeCode.Int32);
            ...
            var genericTypeDefinition = currType.GetGenericTypeDefinition();
