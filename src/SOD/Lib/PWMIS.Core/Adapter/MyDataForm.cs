    ...
    public abstract class MyDataForm
    {
        ...
        protected void AutoUpdateIBFormDataInner(List<IBCommand> ibCommandList)
        {
            ...
            foreach (object item in ibCommandList)
            {
                ...
                {
                    object id = 0;
                    result = DAO.ExecuteInsertQuery(command.InsertCommand, CommandType.Text, command.InsertParameters,
                        ref id, null);
                    if (id != DBNull.Value)
                        try
                        {
        ...
        {
            var guidObj = pkControl.GetValue();
            ...
            foreach (var command in ibCommandList)
                ...
                {
                    var sql = "SELECT [" + pkControl.LinkProperty + "] FROM [" + pkControl.LinkObject + "] WHERE [" +
                              pkControl.LinkProperty + "] = '" + guidText + "'";
        ...
        protected void AutoSelectIBFormInner(List<IDataControl> IBControls)
        {
            ...
            foreach (var obj in IBControls) IBControls2.Add(obj);
            //IBControls2 将会被请空
        ...
        protected int AutoDeleteIBFormInner(List<IDataControl> IBControls)
        {
            ...
            var ibCommandList = GetSelectAndDeleteCommand(IBControls2);
            var count = 0;
            foreach (var command in ibCommandList) count += DAO.ExecuteNonQuery(command.DeleteCommand);
            return count;
        }
        ...
        protected static List<IBCommand> GetIBFormDataInner(List<IDataControl> IBControls, CommonDB DB)
        {
            ...
            while (IBControls.Count > 0)
            {
                ...
                var paraUpdateList = new List<IDataParameter>(); //更新，删除参数列表
                var paraInsertList = new List<IDataParameter>(); //插入参数列表
                var paraPKs = new List<IDataParameter>(); //主键参数列表
                        ...
                        if (ibCtr != null)
                        {
                            ...
                            if (strTableName == ibCtr.LinkObject && ibCtr.LinkProperty != "")
                            {
                                ...
                                if (ctlValue != DBNull.Value)
                                {
                                    ...
                                    var para = DB.GetParameter(ctlParaName, ctlValue);
