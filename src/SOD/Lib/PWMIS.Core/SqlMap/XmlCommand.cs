...
namespace PWMIS.DataMap.SqlMap
{
    ...
    public class XmlCommand
    {
        ...
        private static readonly Dictionary<string, string> dictCacheSqlText = new Dictionary<string, string>();
        private string _CommandGroupName;
        private DBMSType _ScriptType;

        //缓存的配置文件
        private string strBaseNodePath;

        //实际查找的路径
        private string strOldBaseNodePath;
        //基本查找路径模板
        ...
        public DBMSType ScriptType
        {
            ...
            {
                _ScriptType = value;
                strOldBaseNodePath = "/SqlMap/Script[@Type='" + _ScriptType +
                                     "']/CommandClass[@Name='@@CommandGroupName']";
            }
        }
        ...
        {
            var at = CommandName.IndexOf('.');
            if (at != -1)
            {
                //包含命令组名称
                CommandClassName = CommandName.Substring(0, at); //VB: Strings.Left(CommandName, at);
                CommandName = CommandName.Substring(at + 1); //VB: Strings.Mid(CommandName, at + 2);
            }

            ErrDescription = string.Empty;
            if (CommandClassName == string.Empty)
            {
                ErrDescription = "未设置命令组(class)名称！";
                return "";
            }
            ...
            try
            {
                ...
                if (SqlTextPath.StartsWith("@R://"))
                {
                    ...
                    var source = SqlTextPath.Substring(5).Split(','); //程序集+SQL-MAP配置文件成对出现
                    if (source.Length != 2)
                        throw new Exception("嵌入的SQL-MAP资源文件的申明应该以 \"程序集名称,程序集默认名称空间.SqlMap文件名.config\" 的格式 ");
                    var xmlString = string.Empty;
                    if (dictCacheSqlText.ContainsKey(SqlTextPath)) //SqlMap文件可能在一个程序集中
                    {
                        //从缓存读取
                        xmlString = dictCacheSqlText[SqlTextPath];
                    }
                    else
                    {
                        //Assembly ass = Assembly.Load(source[0]);
                        ...
                        //xmlString = sr.ReadToEnd();
                        //sr.Close();
                        xmlString = CommonUtil.GetAssemblyResource(source[0], source[1]);
                        dictCacheSqlText[SqlTextPath] = xmlString;
                    }
                ...
                else
                {
                    ...
                    if (Cache_SqlText.Length == 0)
                    {
                        var sr = new StreamReader(SqlTextPath);
                ...
                var objPath = strBaseNodePath + "/*[@CommandName='" + CommandName + "']";
                ...
                if (SqlText != null && SqlText.HasChildNodes)
                {
                    ...
                    CommandType = (CommandType)Enum.Parse(typeof(CommandType), node.Attributes["CommandType"].Value);
                    QueryType = node.Name;
                    //.Attributes("QueryType").Value
                    return node.InnerText;
                }
