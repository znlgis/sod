    ...
    public class EntityContainer
    {
        ...
        public IEnumerable<T> Map<T>() where T : EntityBase, new()
        {
            ...
            if (Values == null)
            {
                ...
                if (OQL.selectedFieldInfo.Count == 0)
                {
                    var fieldCount = 0;
                    foreach (var entity in OQL.GetAllUsedEntity())
                        for (var i = 0; i < entity.PropertyNames.Length; i++)
                        {
                            var value = entity[i]; //模拟调用
                            fieldCount++;
                        }

                    OQL.Select(new object[fieldCount]);
                }
            ...
            if (Values != null && FieldNames != null)
            {
                ...
                var fieldInfo = new TableNameField[OQL.selectedFieldInfo.Count];
                //查找字段匹配情况
                //entity.PropertyNames 存储的仅仅是查询出来的列名称
                for (var i = 0; i < OQL.selectedFieldInfo.Count; i++)
                {
                    var tnf = OQL.selectedFieldInfo[i];
                ...
                foreach (var itemValues in Values)
                {
                    ...
                    for (var m = 0; m < fieldInfo.Length; m++)
                    {
                        //将容器的值赋值给实体的值元素
                        var entity = fieldInfo[m].Entity;
                        if (entity.GetType() == entityType)
                        {
                            itemEntity = entity;
                            var index = fieldInfo[m].Index;
                    ...
                    var resultEntity = (T)itemEntity.Clone();
        ...
        public IList<TResult> MapToList<T, TResult>(T para, ECMapFunc<T, TResult> fun)
            ...
            where TResult : class
        {
            OQL.fieldStack.Clear();
            var result = fun(para);

            var count = OQL.fieldStack.Count;
            OQL.Select(new object[count]);

            //已经获取到自定义实体类对象中选择的字段，可以用此OQL进行查询了
            var resultList = new List<TResult>();
            foreach (var entity in Map<T>())
            {
                var item = fun(entity);
                resultList.Add(item);
            }
            ...
            where TResult : class
        {
            ...
            OQL.fieldStack.Clear(); //清除可能的调试信息
            var result = fun();
            ...
            var resultList = new List<TResult>();
            foreach (var data in MapMoreEntity(OQL.GetAllUsedEntity()))
            {
        ...
        public DataTable MapToDataTable(string tableName)
        {
            ...
            if (Values != null && FieldNames != null)
            {
                ...
                foreach (var itemValues in Values) dt.Rows.Add(itemValues);
                return dt;
            }
