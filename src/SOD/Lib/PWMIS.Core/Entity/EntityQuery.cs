...
namespace PWMIS.DataMap.Entity
{
    ...
    public class EntityQuery<T> where T : EntityBase, new()
    {
        ...
        public List<T> FillEntityList(OQL oql, T entity)
        {
            ...
            if (entity.PropertyChangedList.Count > 0)
            {
                ...
                var reader = DefaultDataBase.ExecuteDataReader(sql, CommandType.Text, paras);
        ...
        {
            var sql = oql.ToString();
            var Parameters = oql.Parameters;
            var index = 0;
            var condition = string.Empty;

            //为DateTime类型增加
            var tempCount = 0;
            foreach (var temp in entity.PropertyChangedList)
            ...
            if (Parameters == null || Parameters.Count < 1) //不存在Where
            {
                ...
                foreach (var key in entity.PropertyChangedList)
                {
                    var para = DefaultDataBase.GetParameter();
                    var paraName = DefaultDataBase.GetParameterChar + "P" + index;
                    ...
                    else
                    {
                        if (entity.PropertyList(key).GetType() == typeof(DateTime))
                        {
                            ...
                            paraName = DefaultDataBase.GetParameterChar + "P" + index;
                            condition += " AND [" + key + "] < " + paraName;
                            para.ParameterName = paraName;
                ...
                var tableName = entity.GetSchemeTableName();
                var whereIndex = sql.IndexOf(tableName);
                sql = sql.Substring(0, whereIndex + tableName.Length) + (string.IsNullOrEmpty(condition)
                                                                          ? " "
                                                                          : " WHERE " +
            ...
            {
                var fieldCount = Parameters.Count;
            ...
            if (oql.PageEnable)
                switch (db.CurrentDBMSType)
                {
                    ...
                    case DBMSType.SqlServerCe:
                        ...
                        {
                            sql = SQLPage.MakeSQLStringByPage(DBMSType.SqlServer, sql, "", oql.PageSize, oql.PageNumber,
                                oql.PageWithAllRecordCount);
                        }
                        else
                        {
                        ...
                        break;
                    default:
                        sql = SQLPage.MakeSQLStringByPage(db.CurrentDBMSType, sql, "", oql.PageSize, oql.PageNumber,
                            oql.PageWithAllRecordCount);
                        break;
                }
        ...
        {
            return Instance.FillEntity(entity);
        }
        ...
        {
            return Instance.FillEntityList(oql, entity);
        }
        ...
        {
            return QueryObject(oql, DefaultDataBase);
        }
        ...
        {
            return QueryList(oql, DefaultDataBase);
        }
        ...
        public static List<T> QueryList(OQL oql, AdoHelper db)
        {
            ...
            if (oql.PageEnable)
                if (oql.PageWithAllRecordCount == 0)
                {
                    ...
                    oql.PageWithAllRecordCount =
                        CommonUtil.ChangeType<int>(EntityQueryAnonymous.ExecuteOQLCount(oql, db));
                    //如果记录总数仍然是0，直接返回空集合。
                    //感谢网友 koumi 发现此问题。
                    if (oql.PageWithAllRecordCount == 0)
                        return new List<T>();
            ...
            var reader = EntityQueryAnonymous.ExecuteDataReader(oql, db, typeof(T));
            return QueryList(reader, oql.GetEntityTableName());
        }
        ...
        {
            var si = SqlInfo.GetFromCache(cacheKey);
            if (si == null)
            {
                var q = oqlFun(paraValueObject);
                si = EntityQueryAnonymous.GetSqlInfoFromOQL(q, db, typeof(T), false);
                SqlInfo.AddToCache(cacheKey, si);
            }

            var reader = EntityQueryAnonymous.ExecuteDataReader(si, db, false);
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        public static List<T> QueryList(IDataReader reader)
        {
        ...
        /// <returns>实体类集合</returns>
        public static List<T> QueryList(IDataReader reader, string tableName)
        {
            var list = new List<T>();
            if (reader == null)
                return list;
        ...
        public static List<T> QueryList(ConditionHandle<T> condition)
        {
            var db = MyDB.Instance;
            var entity = new T();
            var sql = "SELECT " + string.Join(",", CommonUtil.PrepareSqlFields(entity.PropertyNames)) + " FROM " +
                      entity.GetSchemeTableName();
            var reader = db.ExecuteDataReader(sql);
            var list = new List<T>();
            using (reader)
            {
                var fcount = entity.PropertyNames.Length;
                while (reader.Read())
                {
                    var values = new object[fcount];
                    reader.GetValues(values);
        ...
        public static IEnumerable<T> QueryEnumerable(IDataReader reader, string tableName)
        {
            using (reader)
            {
                if (reader.Read())
        ...
        private List<TChild> QueryChild<TChild>(IEnumerable<T> entitys, AdoHelper db) where TChild : EntityBase, new()
        {
            ...
            var sql = "SELECT * FROM " + child.GetTableName() + " WHERE " + child.GetForeignKey<T>() + " IN ({0})";
            var paraNames = new List<string>();
            var paras = new List<IDataParameter>();
            var count = 0;
            foreach (EntityBase e in entitys)
            {
                var name = db.GetParameterChar + "P" + count;
                paraNames.Add(name);
        ...
        {
            var childList = QueryChild<TChild>(entitys, db);
            var childName = GetChildEntityListPropertyName<TChild>();
            var drm = new DelegatedReflectionMemberAccessor();
            foreach (var e in entitys)
            {
                var newChilds = new List<TChild>();
        ...
        {
            var keyTemp = "QCR_" + typeof(T).FullName + "_";
            foreach (var name in childPropNames)
            {
                var cache = MemoryCache<object>.Default;
                var key = keyTemp + name; //修正原来只能有一个子实体类集合的问题
                var cacheValue = cache.Get(
                    key,
                    p => GetRealQueryAndSetChild(p),
                    name);

                var handle = (RuntimeHandle)cacheValue;

                var realMethod = (MethodInfo)MethodBase.GetMethodFromHandle(handle.MethodHandle, handle.TypeHandle);
                var d = Delegate.CreateDelegate(typeof(MyAction<List<T>, AdoHelper>), this, realMethod)
        ...
        private RuntimeHandle GetRealQueryAndSetChild(string propName)
        {
            ...
            var propType = typeof(T).GetProperty(propName).PropertyType.GetGenericArguments()[0];
            ...
            var typeHandle = thisType.TypeHandle;
        ...
        {
            var result = QueryList(oql, db);
            if (result.Count > 0)
            {
                var query = new EntityQuery<T>();
                //下面采取缓存方式获取属性名字数组
                var cache = MemoryCache<object>.Default;
                var key = "CELPN_" + typeof(T).FullName;
                var cacheValue = cache.Get(
                    key,
        ...
        {
            var fieldCount = entity.PropertyNames.Length;
            if (fieldCount == 0)
        ...
        public int Insert(List<T> entityList)
        {
            ...
            if (entityList.Count > 0)
            {
                ...
                var db = DefaultNewDataBase; //原来是 DefaultDataBase;
                db.BeginTransaction();
                try
                {
        ...
        public int QuickInsert(List<T> entityList)
        {
            ...
            if (entityList.Count > 0)
            {
                var db = DefaultNewDataBase;

                //
                EntityBase entity = entityList[0];
                var objFields = entity.PropertyChangedList;
                if (objFields.Count == 0)
                    throw new Exception("QuickInsert 要求集合的第一个实体类的属性值更改的数量不少于一个！");
                db.ContextObject = entity;

                var paras = new IDataParameter[objFields.Count];

                var tableName = entity.TableName;
                var identityName = entity.IdentityName;
                ...
                foreach (var field in objFields)
                    ...
                    {
                        fields += ",[" + field + "]";
                        var paraName = db.GetParameterChar + "QIP0" + index;
                        values += "," + paraName;
                ...
                {
                    var parasList = new List<IDataParameter>();
                    var batchSize = 10;

                    var num = 0;
                    var valuesTemp = "";
                    var sqlBatch = sqlBatchTemp;
                    ...
                    try
                    {
                        foreach (var item in entityList)
                        {
                            ...
                            for (var i = 0; i < paras.Length; i++)
                            {
                                if (paras[i] == null)
                                    break;
                                var paraName = db.GetParameterChar + "QIP" + num + i;
                                var field = paras[i].SourceColumn;
                                var paraTemp = db.GetParameter(paraName, item.PropertyList(field));
                        ...
                        foreach (var item in entityList)
                        {
                            for (var i = 0; i < paras.Length; i++)
                                if (paras[i] != null)
                                    ...
                                    paras[i] = db.GetParameter(paras[i].ParameterName,
                                        item.PropertyList(paras[i].SourceColumn));
        ...
        public int Delete(List<T> entityList)
        {
            ...
            try
            {
                foreach (var entity in entityList) count += EntityQuery.DeleteInnerByDB(entity, db);
                db.Commit();
            }
            catch (Exception ex)
            {
        ...
        public int ExecuteInsrtOql(OQL q)
        {
            ...
            try
            {
                var count = ExecuteOql(q, DefaultDataBase);
                var result = 0;
                var ec = new EntityCommand(q.currEntity, DefaultDataBase);
                var insertKey = ec.GetInsertKey();
        ...
        private bool isNew; //当前实体是否是一个新实体（对应数据库而言）
        private readonly List<string> changedFields = new List<string>();
        private readonly List<string> selectFields = new List<string>();
        ...
        {
            var objFields = fields.Length > 0 ? selectFields : changedFields;
            if (objFields.Count == 0)
                return 0;
        ...
        public int SaveAllChanges(CommonDB db)
        {
            ...
            return InnerSaveAllChanges(db, CurrentEntity.PropertyChangedList);
        }
        ...
        private int InnerSaveAllChanges(CommonDB db, List<string> objFields)
        {
            ...
            if (isNew)
            {
                count = EntityQuery.InsertInner(CurrentEntity, objFields, db);
                if (count > 0)
                    isNew = false; //保存成功，当前不再是新实体，下次保存应该使用修改
            }
            else
            {
                count = EntityQuery.UpdateInner(CurrentEntity, objFields, db);
            }
    ...
    public class EntityQuery : IEntityQuery
    {
        ...
        /// <returns>实体类集合</returns>
        public static List<T> QueryList<T>(IDataReader reader, string tableName) where T : class
        {
            ...
            using (reader)
            {
                if (reader.Read())
                {
                    ...
                    var t = EntityBuilder.CreateEntity<T>();
                    if (t is EntityBase)
                    {
                        var entity = t as EntityBase;
                        ...
                        do
                        {
                            ...
                            var entityNew = (EntityBase)entity.Clone(false);
        ...
        public static T QueryObject<T>(IDataReader reader) where T : class
        {
            using (reader)
            {
                if (reader.Read())
        ...
        {
            var reader = EntityQueryAnonymous.ExecuteDataReader(oql, db, typeof(T), true);
        ...
        internal static void SetParameterSize(ref IDataParameter para, EntityBase entity, string field, CommonDB DB)
        {
            if (para.Value != null && para.Value.GetType() == typeof(string))
            {
                ...
                if (size > 0) //==0 可能是varcharmax 或者 text类型的字段
                {
                    ...
                    var paratemp = (IDbDataParameter)para;
        ...
        internal static int DeleteInnerByDB(EntityBase entity, CommonDB DB)
        {
            ...
            var paras = new IDataParameter[fieldCount];
            ...
            {
                var paraName = DB.GetParameterChar + "P" + index;
                condition += " AND [" + key + "]=" + paraName;
                paras[index] = DB.GetParameter(paraName, entity.PropertyList(key));
                index++;
            }
            ...
            var count = DB.ExecuteNonQuery(sql, CommandType.Text, paras);
            return count;
        }
        ...
        internal static bool ExistsEntity(EntityBase entity, CommonDB DB)
        {
            ...
            var obj = DB.ExecuteScalar(sql, CommandType.Text, paras);
        ...
        internal static bool FillEntity(EntityBase entity, CommonDB DB)
        {
            ...
            if (fieldCount == 0)
                throw new Exception("EntityQuery Error:实体类属性字段数量为0");
            DB.ContextObject = entity;
            var paras = new IDataParameter[entity.PrimaryKeys.Count];
            var sql = "SELECT ";
            //string fields = "";
            var condition = "";
            var index = 0;
            ...
            sql = sql + string.Join(",", CommonUtil.PrepareSqlFields(entity.PropertyNames)) + " FROM [" +
                  entity.TableName + "] WHERE " + condition.Substring(" AND ".Length);
            var reader = DB.ExecuteDataReader(sql, CommandType.Text, paras);
        ...
        internal static int InsertInner(EntityBase entity, List<string> objFields, CommonDB DB)
        {
            ...
            foreach (var field in objFields)
                if (identityName != field)
                {
                    ...
                    //从缓存中获取当前field所对应的类型
                    var fieldType = entityFieldsCache.GetPropertyType(field);
            ...
            {
                //有自增字段
                var id = entity.PropertyList(identityName);

                var ec = new EntityCommand(entity, DB);
                var insertKey = ec.GetInsertKey();
        ...
        internal static int UpdateInner(EntityBase entity, List<string> objFields, CommonDB DB)
        {
            ...
            if (entity.PrimaryKeys.Count == 0)
                throw new Exception("EntityQuery Error:当前实体类未指定主键字段");
            var fieldCount = objFields.Count + entity.PrimaryKeys.Count;
            if (fieldCount == 0)
                throw new Exception("EntityQuery Error:实体类属性字段数量为0");
            ...
            if (condition == "")
                foreach (var key in entity.PrimaryKeys)
                {
                    ...
                    //从缓存中获取当前field所对应的类型
                    var fieldType = entityFieldsCache.GetPropertyType(key);
        ...
        private int InnerSaveAllChanges(CommonDB db, List<string> objFields)
        {
            ...
            if (isNew)
            {
                count = InsertInner(currEntity, objFields, db);
                if (count > 0)
                    isNew = false; //保存成功，当前不再是新实体，下次保存应该使用修改
            }
            else
            {
                count = UpdateInner(currEntity, objFields, db);
            }
        ...
        public int Delete(List<EntityBase> entityList)
        {
            ...
            try
            {
                foreach (var entity in entityList) count += DeleteInnerByDB(entity, db);
                db.Commit();
            }
            catch (Exception ex)
            {
        ...
        public int Insert(List<EntityBase> entityList)
        {
            ...
            if (entityList.Count > 0)
            {
                ...
                try
                {
                    foreach (var entity in entityList) count += InsertInner(entity, entity.PropertyChangedList, db);
                    db.Commit();
                }
                catch (QueryException qex)
                {
        ...
        public int SaveAllChanges(CommonDB db)
        {
            return InnerSaveAllChanges(db, currEntity.PropertyChangedList);
        }
        ...
        public int Update(List<EntityBase> entityList)
        {
            ...
            try
            {
                foreach (var entity in entityList) count += UpdateInner(entity, entity.PropertyChangedList, db);
                db.Commit();
            }
            catch (QueryException qex)
            {
