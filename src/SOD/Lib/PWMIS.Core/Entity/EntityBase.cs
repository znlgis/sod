...
namespace PWMIS.DataMap.Entity
{
    ...
    public abstract class EntityBase : INotifyPropertyChanged, ICloneable, IEntity
    {
        ...
        protected internal static Dictionary<string, SimplyField> StringFieldSize =
            new Dictionary<string, SimplyField>();
        ...
        public string GetFieldDescription(string fieldName)
        {
            var index = -1;
            var ef = EntityFieldsCache.Item(GetType());
            for (var i = 0; i < ef.Fields.Length; i++)
            {
                var s = ef.Fields[i];
                if (s == fieldName)
                {
        ...
        /// </summary>
        protected internal string IdentityName { get; set; } = string.Empty;
        ...
        ///     主键字段名称列表
        /// </summary>
        public List<string> PrimaryKeys { protected set; get; } = new List<string>();
