...
namespace PWMIS.Common
{
    ...
    public class SQLPage
    {
        ...
        static SQLPage()
        {
            var sqlVer = ConfigurationManager.AppSettings["SqlServerVersion"];
        ...
        public static void CheckSqlServerVersion(SqlServer server)
        {
        }
            ...
            int AllCount)
        {
            ...
            */
            ...
            //排序语法分析 开始
            var sob = new SqlOrderBuilder(strSQLInfo);
            var point = TextSearchUtil.SearchWordsIndex(strSQLInfo, "order by");
            var iOrderAt = point.A;
            if (iOrderAt == -1 && strSQLType != "Count")
            {
                if (PageNumber == 1)
                    return string.IsNullOrEmpty(strWhere) ? sob.Build(PageSize) : sob.Build(PageSize, strWhere);
            ...
            if (strSQLType == "Mid")
            {
                var recordCount = PageSize * PageNumber;
                var prepareSql = string.IsNullOrEmpty(strWhere)
...
ORDER BY {3}
";
                var pageSql = string.Format(sqlTemp, PageSize,
                    prepareSql,
 ...
 ORDER BY {1}
";
                var recordCount = AllCount - PageSize * (PageNumber - 1);
                if (recordCount <= 0)
                    return string.Empty;
                var prepareSql = string.IsNullOrEmpty(strWhere)
                    ? sob.Build(-recordCount)
                    : sob.Build(-recordCount, strWhere);
                var pageSql = string.Format(sqlTemp,
                    prepareSql,
                    sob.GetOrderExpString());
                return pageSql;
            }
        ...
        {
            var at = source.IndexOf(replaceText, StringComparison.CurrentCultureIgnoreCase);
            if (at == -1)
                return source;
            var newStr = source.Substring(0, at) + targetText + source.Substring(at + replaceText.Length);
            return newStr;
        }
        ...
        /// <param name="AllCount">记录总数</param>
        /// <returns>生成SQL分页语句</returns>
        private static string MakePageSQLStringByOracle(string strSQLInfo, string strWhere, int PageSize,
            int PageNumber, int AllCount)
        {
            if (strWhere != null && strWhere != "")
            {
 ...
 (SELECT temptable.*, rownum r_n FROM  
   ( @@SourceSQL ) temptable Where rownum <= @@RecEnd
            ...
            string PKName, string conditon)
        {
            ...
            {
                var sqlTemplage =
                    "Select top @pageSize @filedList from @table1 where  @conditon order by @PKName desc ";
                return sqlTemplage
...
";
            ...
            string PKName, string conditon)
        {
            ...
            if (pageNum == 1)
            {
                var sqlTemplage = "Select Top @pageSize @filedList From @table1 Where  @conditon Order by @PKName Asc ";
                return sqlTemplage
                    .Replace("@pageSize", pageSize.ToString())
                    .Replace("@filedList", filedList)
                    .Replace("@table1", tableName)
