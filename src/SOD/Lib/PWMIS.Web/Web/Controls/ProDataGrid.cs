...
namespace PWMIS.Web.Controls
{
    ...
    public class ProDataGrid : DataGrid
    {
        ...
        /// </summary>
        [Browsable(false)]
        public string ClientSelectedValue { get; set; } = string.Empty;

        /// <summary>
        ///     客户端多选的时候，是否记录上次选择的值，通常用于多页选择。
        /// </summary>
        [Description("客户端多选的时候，是否记录上次选择的值，通常用于多页选择。")]
        [Bindable(true)]
        [Category("Behavior")]
        [DefaultValue(false)]
        ...
        /// </summary>
        [Description("客户端选择的方式,False=单选,True=多选")]
        [Bindable(true)]
        [Category("Behavior")]
        [DefaultValue(false)]
        ...
        ///     用于选择一行时的用户CSS类名
        /// </summary>
        [Description("用于选择一行时的用户CSS类名")]
        [Bindable(true)]
        [Category("Appearance")]
        [DefaultValue("")]
        public string CssClassRowSelected { get; set; } = string.Empty;

        /// <summary>
        ///     用于鼠标悬浮在一行时的用户CSS类名
        /// </summary>
        [Description("用于鼠标悬浮在一行时的用户CSS类名")]
        [Bindable(true)]
        [Category("Appearance")]
        [DefaultValue("")]
        public string CssClassRowMouseMove { get; set; } = string.Empty;

        /// <summary>
        ///     每一行选择的值，对应于某一个列
        /// </summary>
        [Description("每一行选择的值，对应于某一个列。如果值不为空，优先于SelectedFromCellIndex 属性")]
        [Bindable(true)]
        [Category("Data")]
        [DefaultValue("")]
        public string SelectedFieldValue { get; set; } = string.Empty;

        /// <summary>
        ///     选择值所在的单元格的索引，小于1该属性无效
        /// </summary>
        [Description("选择值所在的单元格的索引，小于1该属性无效")]
        [Bindable(true)]
        [Category("Data")]
        [DefaultValue("1")]
        public int SelectedFromCellIndex { get; set; } = 1;
        ...
        ///     是否显示选择列中的选择控件
        /// </summary>
        [Description("是否显示选择列中的选择控件")]
        [Bindable(true)]
        [Category("Appearance")]
        [DefaultValue(true)]
        public bool ShowcheckControl { get; set; } = true;


        /// <summary>
        ///     选择列标题行文字
        /// </summary>
        [Description("选择列标题行文字")]
        [Bindable(true)]
        [Category("Appearance")]
        [DefaultValue("")]
        ...
        [Category("Appearance")]
        [DefaultValue("")]
        public string CheckItemText { get; set; } = string.Empty;

        /// <summary>
        ///     是否应用默认的样式和脚本设置
        /// </summary>
        [Description("是否应用默认的样式和脚本设置")]
        [Bindable(true)]
        [Category("Appearance")]
        [DefaultValue(true)]
        [DesignOnly(true)]
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            var script = "<script language=\"javascript\">\n " +
                         "<!--\n";
            if (ClientSelectMode) script += "SetCheckValues();\n";
            //if(m_MorePageSeleced)//允许多页选择
            script += "InitLastSelected('" + ClientSelectedValue + "');\n";
            script += "//-->\n</script>\n";
            output.Write(script);
        }
                ...
                if (MorePageSeleced)
                {
                    ...
                    if (CurrentSelectedValue != null && SHValue == "-1") //客户端发生了选择事件
                    {
                        if (LastSelectedValues != "")
                        {
                            ...
                            if (LastSelectedValues != "")
                            {
                                ...
                                ClientSelectedValue = CurrentSelectedValue + "," + LastSelectedValues;
                            }
                            else
                            {
                                ClientSelectedValue = CurrentSelectedValue;
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        if (LastSelectedValues != "" && SHValue == "-1")
                        {
                            ...
                            {
                                LastSelectedValues = LastSelectedValues.Replace(",,", "");
                                ClientSelectedValue = LastSelectedValues;
                            }
                            else
                            {
                                ClientSelectedValue = "";
                    ...
                    Page.ClientScript.RegisterHiddenField("LastSelectedValues", ClientSelectedValue);
                }
                else
                {
                    ClientSelectedValue = CurrentSelectedValue;
        ...
        private void RegisterMyClientScript()
        {
            ...
            //注册选择样式脚本
            var script = "<script language=\"javascript\" src=\"" + ScriptPath +
                         "\" type=\"text/Jscript\"></script>\n " +
                         "<script language=\"javascript\">\n " +
                         "<!--\n" +
                         "cssRowSelected=\"" + CssClassRowSelected + "\";\n" +
        ...
        protected override void OnItemDataBound(DataGridItemEventArgs e)
        {
                ...
                e.Item.ItemType == ListItemType.SelectedItem)
            {
                ...
                if (ClientSelectMode)
                    e.Item.Cells[0].Text =
                        GetSelectTableHTML("checkbox", text,
                            Value); // "<input type='checkbox' name='CID' >" +(e.Item.ItemIndex+1).ToString ();
                else
                    e.Item.Cells[0].Text =
                        GetSelectTableHTML("radio", text,
                            Value); //"<input type='radio' name='CID' >" +(e.Item.ItemIndex+1).ToString ();
                //绑定序号
    ...
    internal class ColumnTemplate2 : ITemplate
    {
        ...
        ///     是否是多选
        /// </summary>
        public bool IsMoreSelect { get; set; } = true;

        /// <summary>
        ///     “选择所有”的文字
        /// </summary>
        public string CheckAllText { set; get; } = "";
