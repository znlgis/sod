...
namespace PWMIS.Core.Extensions
{
    ...
    public abstract class DbContext : IDbContextProvider
    {
        ...
        private static readonly Dictionary<string, bool> dictCheckedDb = new Dictionary<string, bool>();

        //存储已经校验过的实体类的字典，Key为当前DbContext的类型的 RuntimeTypeHandle
        private static readonly Dictionary<RuntimeTypeHandle, List<RuntimeTypeHandle>> dictCheckedEntitys =
            new Dictionary<RuntimeTypeHandle, List<RuntimeTypeHandle>>();
