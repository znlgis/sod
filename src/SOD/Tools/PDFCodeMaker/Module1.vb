...
Module Module1
    ...
    Sub Main()
        Dim VerMsg As String = "PDF.NET Code Maker : " & Assembly.GetEntryAssembly().FullName &
        ...
        Dim Language As CodeLanguage = [Enum].Parse(GetType(CodeLanguage),
                                                    ConfigurationManager.AppSettings("CodeLanguage"))
    ...
    Private Function SaveCode(OutPutFile As String, CodeString As String) As Boolean
        Try
            Dim writer As StreamWriter = New StreamWriter(OutPutFile, False, Encoding.Default)
    ...
    Private Sub WriteLog(Log As String)
        ...
        End If
        Dim writer As StreamWriter = New StreamWriter("LOG\Make.Log", True, Encoding.Default)
        writer.WriteLine(Log)
        writer.Flush()
        writer.Close()
    End Sub

    Function MakeCodeString(SqlMapConfigFile As String, RootNameSpace As String, OutPutPath As String,
                            Optional ByVal ReWrite As Boolean = True,
                            Optional ByVal Language As CodeLanguage = CodeLanguage.VB) As Integer
        '处理输出目录
        ...
        End If
        Dim reader As StreamReader = New StreamReader(CodeTemplateFileName, Encoding.Default)
        ...
        If Not Scripts Is Nothing Then
            For Each Script As XmlNode In Scripts
                If Not Script.Attributes("Type") Is Nothing AndAlso Not Script.Attributes("Version") Is Nothing Then
                    ...
                    If Not Script.Attributes("ConnectionString") Is Nothing Then _
                        connectionString = Script.Attributes("ConnectionString").Value
                    Dim providerName As String = ""
                    If Not Script.Attributes("ProviderName") Is Nothing Then _
                        providerName = Script.Attributes("ProviderName").Value
                    ...
                    Dim DB As AdoHelper = MyDB.GetDBHelperByProviderString(providerName, connectionString)
                    ...
                    If Script.HasChildNodes Then
                        ...
                        For Each CommandClass As XmlNode In Script.ChildNodes
                            ...
                            GetStringArea(CodeString, "<%R:REM[", "REM]%>",
                                          "使用该程序前请先引用程序集：PWMIS.Core，并且下面定义的名称空间前缀不要使用ＰＷＭＩＳ，更多信息，请查看 http://www.pwmis.com/sqlmap")
                            Dim FunTemplateString As String = GetStringArea(CodeString, "<%R:Function[", "Function]%>",
                                                                            "@FunTemplate") _
                            'Mid(CodeString, ReFunAtB, ReFunAtE - ReFunAtB)
                            ...
                            If _
                                Not CommandClass.Attributes("Class") Is Nothing AndAlso
                                CommandClass.Attributes("Class").Value <> "" Then
                            ...
                            If _
                                Not CommandClass.Attributes("Interface") Is Nothing AndAlso
                                CommandClass.Attributes("Interface").Value <> "" Then
                            ...
                            If _
                                Not CommandClass.Attributes("Description") Is Nothing AndAlso
                                CommandClass.Attributes("Description").Value <> "" Then
                            ...
                            '执行主类文本替换
                            CodeString =
                                CodeString.Replace("#CurrNameSpace#", CurrNameSpace).Replace("#CommandClassMap#",
                                                                                             CommandClassMap).Replace(
                                                                                                 "#CommandClassName#",
                                                                                                 CommandClassName)
                            CodeString =
                                CodeString.Replace("#CreateDateTime#", DateTime.Now.ToString()).Replace(
                            ...
                            End If
                            CodeString =
                                CodeString.Replace("#CommandClassInterface#", CommandClassInterface).Replace(
                                    "#EmbedAssemblySource#", EmbedAssemblySource)
                            ...
                            If CommandClass.HasChildNodes Then
                                ...
                                For Each node As XmlNode In CommandClass.ChildNodes
                                    ...
                                    If _
                                        Not node.Attributes("Method") Is Nothing AndAlso
                                        node.Attributes("Method").Value.Trim() <> "" Then
                                    ...
                                    If _
                                        Not node.Attributes("ResultClass") Is Nothing AndAlso
                                        node.Attributes("ResultClass").Value <> "" Then
                                        ResultClass = node.Attributes("ResultClass").Value
                                        '结果类型为实体类型方可指定结果映射类型
                                        If ResultClass = enumResultClass.EntityObject.ToString() Or
                                           ResultClass = enumResultClass.[EntityList].ToString() Or
                                           ResultClass = enumResultClass.ObjectList.ToString() Then
                                            If _
                                                Not node.Attributes("ResultMap") Is Nothing AndAlso
                                                node.Attributes("ResultClass").Value <> "" Then
                                    ...
                                    If _
                                        Not node.Attributes("Description") Is Nothing AndAlso
                                        node.Attributes("Description").Value <> "" Then
                                    ...
                                    Dim DataParameters() As IDataParameter = cmdInfo.DataParameters _
                                    ' mapper.GetParameters(SqlMapScript)
                                    Dim ArrFunTemplateString() As String = FunTemplateString.Split(vbCrLf) _
                                    '.ToCharArray()

                                    '生成函数说明
                                    ArrFunTemplateString(2) = ArrFunTemplateString(2).Replace("#CommandDescription#",
                                                                                              CommandDescription)
                                    ...
                                    For I As Integer = 0 To ParamsLength
                                        tempStr &= ArrFunTemplateString(5).Replace("#ParameterName#",
                                                                                   mapper.ParamsMap(I).ParamName)
                                    Next
                                    ArrFunTemplateString(5) = tempStr

                                    '替换函数参数
                                    Dim FunParamsTemplate As String = GetStringArea(ArrFunTemplateString(8),
                                                                                    "<%R:Params[", "Params]%>",
                                                                                    "@FunParams")
                                    ...
                                    For I As Integer = 0 To ParamsLength
                                        tempStr &=
                                            FunParamsTemplate.Replace("#ParameterName#", mapper.ParamsMap(I).ParamName).
                                                Replace("#DataType#", mapper.ParamsMap(I).TypeCode.ToString())
                                    ...
                                    If mapper.ParamsReplaceable.Count > 0 Then
                                        ...
                                        For I As Integer = 0 To ReplaceCount
                                            tempStr &= FunParamReplaceText.Replace("#ReplaceableParameter#",
                                                                                   mapper.ParamsReplaceable(I))
                                    ...
                                    Select Case nodeName
                                        Case "Select", "Read"
                                            Select Case ResultClass
                                                Case "", enumResultClass.Default.ToString(),
                                                    enumResultClass.DataSet.ToString()
                                                ...
                                                Case enumResultClass.EntityObject.ToString() '实体类型
                                                    If ResultClass = "" Then
                                                        Throw _
                                                            New Exception(
                                                                "函数返回值指定错误，不能指定一个空的结果实体！错误所在位置：" &
                                                                ArrFunTemplateString(8))
                                                    End If
                                        ...
                                        Case Else
                                            Throw _
                                                New Exception(
                                                    "不支持的CRUD类型，请指定Create/Insert,Read/Select,Update,Delete 之一！")
                                    End Select

                                    '替换函数返回类型
                                    ArrFunTemplateString(8) = ArrFunTemplateString(8).Replace("#ResultClass#",
                                                                                              ResultClass)

                                    '替换命令名
                                    ArrFunTemplateString(11) = ArrFunTemplateString(11).Replace("#CommandName#",
                                                                                                CommandName)
                                    ...
                                    For I As Integer = 0 To ParamsLength
                                        tempStr &=
                                            ArrFunTemplateString(14).Replace("#ParamIndex#", I.ToString).Replace(
                                                "#ParameterName#", mapper.ParamsMap(I).ParamName)
                                    Next
                                    ...
                                    tempStr = String.Empty
                                    For I As Integer = 0 To ParamsLength
                                        tempStr &= ArrFunTemplateString(18).Replace("#ParameterName#",
                                                                                    mapper.ParamsMap(I).ParamName)
                                    Next
                                    ...
                                    If mapper.ParamsReplaceable.Count > 0 Then
                                        For I As Integer = 0 To mapper.ParamsReplaceable.Count - 1
                                            tempStr &= ArrFunTemplateString(21).Replace("#ReplaceableParameter#",
                                                                                        mapper.ParamsReplaceable(I))
                                        Next
                                    End If
                                    ArrFunTemplateString(21) = tempStr

                                    ArrFunTemplateString(23) = ArrFunTemplateString(23).Replace("#ExecuteType#",
                                                                                                ExecuteType)
                                    '替换查询方式
                                    If Language = CodeLanguage.VB Then
                                        If ParamsLength < 0 Then
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("#DataParameters#", ",Nothing")
                                        Else
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("#DataParameters#",
                                                                                 ", cmdInfo.DataParameters")
                                        End If
                                        If ExecuteType = "ExecuteReader" Then
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("Return", "Return .ResultMapEntity(") +
                                                ", New " & ResultClass & " )"
                                        ElseIf ExecuteType = "ExecuteReaderList" Then
                                            ArrFunTemplateString(23) = ArrFunTemplateString(23).Replace("Return",
                                                                                                        "Return EntityQuery(Of " &
                                                                                                        ResultMap &
                                                                                                        ").QueryList(")
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("ExecuteReaderList", "ExecuteReader") &
                                                ")"
                                        ElseIf ExecuteType = "ExecuteObjectList" Then
                                            ArrFunTemplateString(23) = ArrFunTemplateString(23).Replace("Return",
                                                                                                        "Return MapObjectList(Of " &
                                                                                                        ResultMap & ")(")
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("ExecuteObjectList", "ExecuteReader") &
                                                ")"
                                        End If
                                    ElseIf Language = CodeLanguage.CSharp Then
                                        If ParamsLength < 0 Then
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("#DataParameters#", ",null")
                                        Else
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("#DataParameters#",
                                                                                 ", cmdInfo.DataParameters")
                                        ...
                                        If ExecuteType = "ExecuteReader" Then
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace("return",
                                                                                 "return (" & ResultClass &
                                                                                 ")Mapper.ResultMapEntity(") + ", new " &
                                                ResultClass & "() );"
                                        ...
                                        ElseIf ExecuteType = "ExecuteReaderList" Then
                                            ArrFunTemplateString(23) = ArrFunTemplateString(23).Replace("return",
                                                                                                        ...
                                                                                                        ">.QueryList(")
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace(");", "));").Replace(
                                                    "ExecuteReaderList", "ExecuteReader")
                                        ElseIf ExecuteType = "ExecuteObjectList" Then
                                            ArrFunTemplateString(23) = ArrFunTemplateString(23).Replace("return",
                                            ...
                                            ArrFunTemplateString(23) =
                                                ArrFunTemplateString(23).Replace(");", "));").Replace(
                                                    "ExecuteObjectList", "ExecuteReader")
                                        End If
                                    Else
                                        '
                                    End If
                                    ...
                                    '生成完整的函数体
                                    Dim FullFunBody As New StringBuilder
                                    With FullFunBody
                                        ...
                                        If mapper.ParamsReplaceable.Count > 0 Then .Append(ArrFunTemplateString(20)) _
                                        '.Append(vbLf)
                            ...
                            End If
                            Dim OutPutFile As String = CurrOutPutPath &
                                                       OutFileName.Replace("#OutPutFile#", CommandClassName)
    ...
    Private Function GetStringArea(ByRef Source As String, BeginFlag As String, EndFlag As String,
                                   ReplacedFlag As String) As String
    ...
    Private Function GetEmbedAssemblySource(ConfigFile As String) As String
        Dim doc As XmlDocument = New XmlDocument
        Dim _ErrDescription As String
        Try
            doc.Load(ConfigFile) '可以考虑采用XML文件流的方式加快读取
