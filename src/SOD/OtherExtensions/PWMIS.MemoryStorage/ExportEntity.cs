...
namespace SOD.DataSync
{
    ...
    public class ExportEntity<T> where T : EntityBase, new()
    {
        ...
        private void SaveEntity(T[] entitys, ExportEntityEventArgs<T> args)
        {
            if (entitys.Length > 0)
            {
                args.Succeed = MemDB.SaveEntity(entitys);
            }
            else
            {
                MemDB.DropEntity<T>();
        ...
        public void Export(Func<int, T, OQL> funQ, Func<T, int> initBatchNumber)
        {
            ...
            {
                //导出批次管理
                var exportTableName = EntityFieldsCache.Item(entityType).TableName;
                var batchList = MemDB.Get<ExportBatchInfo>();
                var currBatch = batchList.FirstOrDefault(p => p.ExportTableName == exportTableName);
                if (currBatch == null)
                {
                ...
                //导出数据
                var q = funQ(currBatch.BatchNumber, new T());
                var entityList = q != null ? CurrDbContext.QueryList<T>(q) : CurrDbContext.QueryAllList<T>();
                var args = new ExportEntityEventArgs<T>(entityList, entityType, exportTableName);
                args.Succeed = true;
            ...
            {
                var args = new ExportEntityEventArgs<T>(null, entityType, EntityFieldsCache.Item(entityType).TableName);
                args.Succeed = false;
